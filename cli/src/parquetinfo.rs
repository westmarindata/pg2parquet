...
fn print_col_info<T: DataType<T = T2>, T2: Default + Clone + std::fmt::Debug>(
    col_name: &str,
    col: &mut ColumnReaderImpl<T>,
) {
    let batch_size = 300;
    let mut data: Vec<T2> = vec![<T as DataType>::T::default(); batch_size];
    let mut dls = vec![0; batch_size];
    let mut rls = vec![0; batch_size];
    let (valuecount, totalcount) = col
        .read_batch(batch_size, Some(&mut dls), Some(&mut rls), &mut data)
        .unwrap();
