use parquet::basic::{self, Compression, ConvertedType, Encoding, Repetition};
...
pub fn create_something(file_path: &std::path::PathBuf) {
    ...
    let schema = Type::group_type_builder("root")
        .with_fields(&mut vec![
            Type::primitive_type_builder("id", basic::Type::INT32)
                .build()
                .unwrap()
                .arc(),
    ...
    col_id
        .typed::<Int32Type>()
        .write_batch(&vec![1, 0, 2], Some(&vec![1, 0, 1]), None)
        .unwrap();
    col_id.close().unwrap();
    ...
    y.typed::<Int64Type>()
        .write_batch(&vec![now_ms, 0, 0], Some(&vec![2, 1, 0]), None)
        .unwrap();
    y.close().unwrap();
    rg.close().unwrap();
    writer.close().unwrap();
