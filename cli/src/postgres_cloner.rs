...
use postgres::{self, Client, Column, NoTls, Row, RowIter, Statement};

use crate::column_appender::{
    ArrayColumnAppender, ColumnAppender, GenericColumnAppender, RealMemorySize,
};
use crate::column_pg_copier::{BasicPgColumnCopier, ColumnCopier, MergedColumnCopier};
...
use crate::myfrom::{self, MyFrom};
use crate::parquet_row_writer::{
    ParquetRowWriter, ParquetRowWriterImpl, WriterSettings, WriterStats,
};
use crate::pg_custom_types::{PgAbstractRow, PgEnum, PgRawRange, PgRawRecord};
...
pub fn execute_copy(
    ...
    let rows: RowIter = client
        .query_raw::<Statement, &i32, &[i32]>(&statement, &[])
        .unwrap();
...
fn format_schema(schema: &ParquetType, indent: u32) -> String {
    ...
    let mut additional_info = basic_info
        .logical_type()
        .map(|lt| format!("{:?}", lt))
        .or_else(|| match basic_info.converted_type() {
    ...
    match schema {
        ParquetType::PrimitiveType {
            ...
            let byte_size = if *type_length >= 0 {
                format!(" [{}b]", *type_length)
            } else {
                "".to_string()
            };
        ...
        ParquetType::GroupType { basic_info, fields } => {
            ...
            let rp =
                if basic_info.has_repetition() && basic_info.repetition() != Repetition::REQUIRED {
                    basic_info.repetition().to_string() + " "
                } else {
                    "".to_string()
                };
...
fn map_schema_root(row: &[Column], s: &SchemaSettings) -> Result<ResolvedColumn<Row>, String> {
    ...
    let merged_copier: Box<dyn ColumnCopier<Row>> =
        Box::new(MergedColumnCopier::<Row>::new(column_copiers, 0, 0));
...
fn map_schema_column<TRow: PgAbstractRow>(
    ...
    match t.kind() {
        Kind::Simple => map_simple_type(t, c, s, CreateCopierCallback::new()),
        Kind::Enum(ref _enum_data) => match s.enum_handling {
            SchemaSettingsEnumHandling::Int => Ok(resolve_primitive::<PgEnum, Int32Type, _>(
                c.col_name(),
                c,
                CreateCopierCallback::new(),
                None,
                None,
            )),
            SchemaSettingsEnumHandling::Text => Ok(resolve_primitive::<PgEnum, ByteArrayType, _>(
                c.col_name(),
                c,
                CreateCopierCallback::new(),
        ...
        Kind::Array(ref element_type) => map_schema_column(element_type, &c.as_array(), s),
        Kind::Domain(ref element_type) => map_schema_column(element_type, c, s),
        &Kind::Range(ref element_type) => {
            ...
            let col_lower_incl =
                create_primitive_copier_simple::<bool, BoolType, _>(&c.nest("lower_inclusive", 2));
            let col_upper_incl =
                create_primitive_copier_simple::<bool, BoolType, _>(&c.nest("upper_inclusive", 3));
            let col_is_empty =
                create_primitive_copier_simple::<bool, BoolType, _>(&c.nest("is_empty", 4));
            ...
            let schema = ParquetType::group_type_builder(c.col_name())
                .with_fields(&mut vec![
                    ...
                    Arc::new(
                        ParquetType::primitive_type_builder("is_empty", basic::Type::BOOLEAN)
                            .build()
                            .unwrap(),
                    ),
                ])
            ...
            let copier = create_complex_appender::<PgRawRange, _>(
                c,
                CreateCopierCallback::<TRow>::new(),
                vec![
                    col_lower.0,
                    col_upper.0,
                    col_lower_incl,
        ...
        &Kind::Composite(ref fields) => {
            ...
            let copier = create_complex_appender::<PgRawRecord, _>(
                c,
                CreateCopierCallback::<TRow>::new(),
                column_copiers,
            );
        ...
        _ => Err(format!(
            "Could not map column {}, unsupported type: {}",
            c.full_name(),
            t
        )),
    }
...
fn map_simple_type<Callback: AppenderCallback>(
    ...
    Ok(match t.name() {
        ...
        "int2" => resolve_primitive::<i16, Int32Type, _>(
            name,
            c,
            callback,
            None,
        ...
        "numeric" => {
            ...
            let schema = ParquetType::primitive_type_builder(name, basic::Type::BYTE_ARRAY)
                ...
                .build()
                .unwrap();
            let cp = wrap_appender(
                c,
                callback,
                new_decimal_bytes_appender(
        ...
        "money" => resolve_primitive::<PgMoney, Int64Type, _>(
            name,
            c,
            callback,
        ...
        "char" => resolve_primitive::<i8, Int32Type, _>(
            name,
            c,
            callback,
            None,
        ...
        "name" | "text" | "xml" | "bpchar" | "varchar" | "citext" => {
            resolve_primitive::<String, ByteArrayType, _>(
                name,
                c,
                callback,
                None,
        ...
        "jsonb" | "json" => resolve_primitive::<PgRawJsonb, ByteArrayType, _>(
            name,
            c,
            callback,
            None,
            Some(match s.json_handling {
                SchemaSettingsJsonHandling::Text => ConvertedType::UTF8,
                SchemaSettingsJsonHandling::TextMarkedAsJson => ConvertedType::JSON,
            }),
        ),
        "timestamptz" => resolve_primitive::<chrono::DateTime<chrono::Utc>, Int64Type, _>(
            name,
            c,
            callback,
        ...
        "timestamp" => resolve_primitive::<chrono::NaiveDateTime, Int64Type, _>(
            name,
            c,
            callback,
            Some(LogicalType::Timestamp {
        ...
        "date" => resolve_primitive::<chrono::NaiveDate, Int32Type, _>(
            name,
            c,
            callback,
        ...
        "time" => resolve_primitive::<chrono::NaiveTime, Int64Type, _>(
            name,
            c,
            callback,
        ...
        "uuid" => resolve_primitive::<uuid::Uuid, FixedLenByteArrayType, _>(
            name,
            c,
            callback,
        ...
        "macaddr" => match s.macaddr_handling {
            SchemaSettingsMacaddrHandling::Text => {
                resolve_primitive::<eui48::MacAddress, ByteArrayType, _>(
                    name,
                    c,
                    callback,
                    None,
            ...
            SchemaSettingsMacaddrHandling::ByteArray => {
                resolve_primitive::<eui48::MacAddress, FixedLenByteArrayType, _>(
                    name, c, callback, None, None,
                )
            }
        ...
        "inet" => resolve_primitive::<IpAddr, ByteArrayType, _>(
            name,
            c,
            callback,
            None,
        ...
        "bit" | "varbit" => resolve_primitive::<bit_vec::BitVec, ByteArrayType, _>(
            name,
            c,
            callback,
            None,
        ...
        n => {
            return Err(format!(
                "Could not map column {}, unsupported primitive type: {}",
                c.full_name(),
                n
            ))
        }
...
where
    TDataType: DataType,
    TDataType::T: RealMemorySize + MyFrom<T>,
{
    resolve_primitive_conv::<T, TDataType, _, Callback>(
        name,
        c,
        callback,
        logical_type,
...
fn resolve_primitive_conv<
    T: for<'a> FromSql<'a> + 'static,
    TDataType,
    FConversion: Fn(T) -> TDataType::T + 'static,
...
where
    TDataType: DataType,
    TDataType::T: RealMemorySize,
{
    let mut c = c.clone();
    c.definition_level += 1; // TODO: can we support NOT NULL fields?
    let mut t = ParquetType::primitive_type_builder(name, TDataType::get_physical_type())
        .with_repetition(c.pq_repetition())
    ...
    let cp = create_primitive_appender::<T, TDataType, _, _>(&c, callback, convert);

    (cp, t)
}
fn create_primitive_copier_simple<
    T: for<'a> FromSql<'a> + 'static,
    TDataType,
    TRow: PgAbstractRow,
>(
    ...
    TDataType::T: RealMemorySize + MyFrom<T>,
{
    let mut c = c.clone();
    c.definition_level += 1;
    create_primitive_appender::<T, TDataType, _, _>(&c, CreateCopierCallback::<TRow>::new(), |x| {
        TDataType::T::my_from(x)
    })
}

fn create_primitive_appender<
    T: for<'a> FromSql<'a> + 'static,
    TDataType,
    FConversion: Fn(T) -> TDataType::T + 'static,
    Callback: AppenderCallback,
...
where
    ...
    TDataType::T: RealMemorySize,
{
    let basic_appender: GenericColumnAppender<T, TDataType, _> =
        GenericColumnAppender::new(c.definition_level, c.repetition_level, convert);
    wrap_appender(c, callback, basic_appender)
}

fn create_complex_appender<T: for<'a> FromSql<'a> + 'static, Callback: AppenderCallback>(
    c: &ColumnInfo,
    callback: Callback,
    copiers: Vec<DynCopier<T>>,
) -> Callback::TResult {
    let main_cp = MergedColumnCopier::new(copiers, c.definition_level + 1, c.repetition_level);
    wrap_appender(c, callback, main_cp)
}

fn wrap_appender<T: for<'a> FromSql<'a> + 'static, Callback: AppenderCallback>(
    c: &ColumnInfo,
    callback: Callback,
    appender: impl ColumnAppender<T> + 'static,
) -> Callback::TResult {
    if c.is_array {
...
impl ColumnInfo {
    ...
    fn as_array(&self) -> ColumnInfo {
        assert!(
            self.is_array == false,
            "Parquet does not support nested arrays"
        );
        ColumnInfo {
...
trait AppenderCallback {
    ...
    where
        TAppender: ColumnAppender<TPg> + 'static,
        TPg: for<'a> FromSql<'a> + 'static;
}

struct CreateCopierCallback<TRow> {
    _dummy: PhantomData<TRow>,
}
impl<TRow> CreateCopierCallback<TRow> {
    fn new() -> Self {
        CreateCopierCallback {
            _dummy: PhantomData,
        }
    }
}
