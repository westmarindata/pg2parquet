...
use crate::{
    column_appender::{ColumnAppender, GenericColumnAppender},
    column_pg_copier::DynamicSerializedWriter,
...
pub fn new_decimal_bytes_appender(
    max_dl: i16,
    max_rl: i16,
    precision: u32,
    scale: i32,
) -> impl ColumnAppender<PgNumeric> {
    let inner: GenericColumnAppender<Vec<u8>, ByteArrayType, _> =
        GenericColumnAppender::new(max_dl, max_rl, |v| MyFrom::my_from(v));
...
impl<TInner: ColumnAppender<Vec<u8>>> ColumnAppender<PgNumeric> for DecimalBytesAppender<TInner> {
    fn copy_value(
        ...
        let bytes = value
            .n
            .map(|n| convert_decimal_to_bytes(n, self.scale, self.precision));
