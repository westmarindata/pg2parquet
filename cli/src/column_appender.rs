...
use parquet::data_type::{AsBytes, ByteArray, DataType, SliceAsBytes};
...
pub fn generic_column_appender_new_myfrom<TPg, TPq>(
    max_dl: i16,
    max_rl: i16,
) -> GenericColumnAppender<TPg, TPq, impl Fn(TPg) -> TPq::T>
where
    TPq::T: RealMemorySize,
    TPq: DataType,
    TPq::T: MyFrom<TPg>,
{
...
where
    TPq::T: Clone + RealMemorySize,
    TPq: DataType,
    FConversion: Fn(TPg) -> TPq::T,
{
    max_dl: i16,
    max_rl: i16,
    column: Vec<TPq::T>,
    ...
    FConversion: Fn(TPg) -> TPq::T,
{
    ...
    fn write_column(&mut self, writer: &mut SerializedColumnWriter) -> Result<(), ParquetError> {
        let dls = if self.max_dl > 0 {
            Some(self.dls.as_slice())
        } else {
            None
        };
        let rls = if self.max_rl > 0 {
            Some(self.rls.as_slice())
        } else {
            None
        };
...
impl<TPg, TPq, FConversion> GenericColumnAppender<TPg, TPq, FConversion>
where
    TPq::T: Clone + RealMemorySize + MyFrom<TPg>,
    TPq: DataType,
    FConversion: Fn(TPg) -> TPq::T,
{
    pub fn new_mfrom(
